...
function recordPerf(key, ok) {
  ...
  if (ok) {
    p.r++;
    // Un acierto "perdona" un poco los fallos anteriores, para que un ítem
    // ya dominado deje de considerarse "débil" tras un par de repasos buenos.
    p.w = Math.max(0, p.w - 1);
  } else {
    p.w = Math.min(5, p.w + 1); // limita cuánto puede "pesar" un mismo fallo
  }
  S.perf[key] = p;
  save();
}
...
function bestVoice() {
  ...
  // Primero: cualquier voz local (no de red) cuyo idioma sea español —
  // esto cubre paquetes de voz nuevos descargados en Windows/Mac aunque
  // tengan nombres distintos a los de siempre.
  const esVoices = vv.filter(v => v.lang && v.lang.toLowerCase().startsWith('es'));
  if (esVoices.length) {
    const names = ['Google español','Microsoft Pablo','Microsoft Sabina','Microsoft Helena',
                   'Microsoft Elvira','Microsoft Alvaro','Microsoft Dalia','Microsoft Raul',
                   'Paulina','Monica','Natural'];
    for (const n of names) {
      const v = esVoices.find(x => x.name.includes(n));
      if (v) { _bestVoice = v; return v; }
    }
    _bestVoice = esVoices[0];
    return _bestVoice;
  }
  return null;
}
...
let _lastRepasoGame = null;
let _lastRepasoCons = null;
function renderRepaso() {
  let games = ['silabario','palabras','silabas','silabafalta','iniciales','completa'];
  if ((FRASES[S.lang] || []).length && S.unlocked.includes('frases_1')) games.push('frases');
  if (games.length > 1 && _lastRepasoGame) games = games.filter(g => g !== _lastRepasoGame);
  const game = rand(games);
  _lastRepasoGame = game;

  if (game === 'silabario') {
    const weak = weakConsList();
    let pool = weak.length && Math.random() < 0.5 ? weak : unlockedConsList();
    // Evita mostrar la misma familia dos veces seguidas si hay otras opciones
    if (pool.length > 1 && _lastRepasoCons) {
      const filtered = pool.filter(c => c !== _lastRepasoCons);
      if (filtered.length) pool = filtered;
    }
    window._activeCons = rand(pool);
    _lastRepasoCons = window._activeCons;
  }
  if (game === 'palabras') {
